...
/**
 * Simple JavaCL example that estimates PI by firing darts at a dart board. This
 * is based on the HADOOP PI estimation example.
 * 
 * This uses JavaCL 1.0.0-RC1, which uses the new BridJ API, with the generic
 * buffers, like CLBuffer&lt;Integer&gt;.
 * 
 * @author Mark Utting
 */
public class Pi {
	 ...
	 * 
	 * @param seeds
	 *            one integer seed for each task (work item).
	 * @param repeats
	 *            the number of darts each task must throw.
	 * @param output
	 *            one integer output cell for each task
	 * @param gid
	...
	public static void dummyThrowDarts(int[] seeds, int repeats, int[] output,
		...
		for (int iter = 0; iter < repeats; iter++) {
			...
			x = ((rand & 0xfff) / 4096.0f);
			y = (((rand >> 12) & 0xfff) / 4096.0f);
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 3) {
			...
			System.err
					.println("      (tasks must be a multiple of workgroupsize)");
			System.exit(1);
		}
		...
		for (int h = 0; h < 5; h++) {
			...
			CLBuffer<Integer> memIn1 = context.createIntBuffer(
					CLMem.Usage.Input, tasks);
			CLBuffer<Integer> memOut = context.createIntBuffer(
			...
			CLKernel kernel = program.createKernel("throwDarts", memIn1,
					repeats, memOut);

			// Execute the kernel with global size = dataSize and workgroup size
			// = 1
			System.out.println("Starting with " + tasks + " tasks, each doing "
					+ repeats + " repeats.");
			System.out.flush();
			final long time0 = System.nanoTime();
			kernel.enqueueNDRange(queue, new int[] { tasks },
					new int[] { wgSize });
			...
			for (int i = 0; i < tasks; i++) {
				// System.out.println("task i: " + i + " gives " +
				// output.get(i));
				inside += output.get(i);
			}
			final double pi = 4.0 * inside / total;
			System.out.println("Estimate PI = " + inside + "/" + total + " = "
					+ pi);
		}
	}
}
